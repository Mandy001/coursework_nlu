...
class RNN(object):
    ...
    def predict(self, x):
        ...
        for t in range(len(x)):
            ...
            input = self.V[:, x[t]]
            last_state = s[t - 1]
            s[t] = sigmoid(input + np.dot(self.U, last_state))
    ...
    def acc_deltas(self, x, d, y, s):
        ...
        for t, y_t in enumerate(y):
            state_error = np.zeros(self.hidden_dims)
            for i, pred in enumerate(y_t):
                if i == d[t]:
                    error = pred - 1
                else:
                    error = pred - 0
                error = error * -1
                self.deltaW[i] += s[t] * error
                state_error += error * self.W[i]

            state_logits_error = state_error * s[t] * (1 - s[t])
            self.deltaV[:, x[t]] += state_logits_error
            self.deltaU += np.outer(state_logits_error, s[t-1])


        # target = np.zeros(y.shape)
        # error = y - d
        # self.deltaW += np.dot(error.T, s[:-1])
        # return self.acc_deltas_np(x, d, y, s)
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        # state_error = np.dot(error, self.W)
        # state_logits_error = state_error * s * (1 - s)
        #
        #
        #
        # print(self.deltaW.shape)
        # print(self.state_error.shape)
        #
        # one_hot_x = np.zeros((len(y), self.vocab_size))
        # self.deltaV += np.dot(state_logits_error, )
    ...
    def acc_deltas_bptt(self, x, d, y, s, steps):
        ...
        #     self.deltaV[:, x[t]] += state_logits_error
        #     self.deltaU += np.outer(state_logits_error, s[t - 1])
        #     state_error = np.dot(state_logits_error, self.U)
        ...
        #     error = -(y[t] - target)
        #     self.deltaW += np.outer(error, s[t])
        #     state_error += np.dot(error, self.W)
        #
        #     state_logits_error = state_error * s[t] * (1 - s[t])
        #     self.deltaV[:, x[t]] += state_logits_error
        ...
        #
        #
        #     last_state_error = np.dot(state_logits_error + e, self.U)
    ...
    def compute_mean_loss(self, X, D):
        ...
        for i, x in enumerate(X):
            mean_loss += self.compute_loss(x, D[i])
            total_t += len(D[i])
        mean_loss /= total_t
        return mean_loss
